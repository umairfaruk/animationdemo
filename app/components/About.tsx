    ...
    <section id="about" style={{ background: '#000', overflow: 'hidden' }}>
      ...
      <div style={{
        ...
        <div style={{
          ...
          animation: `${isRtl ? 'marqueeRtl' : 'marquee'} 22s linear infinite`,
          whiteSpace: 'nowrap', width: 'max-content',
        }}>
          {Array.from({ length: 6 }).flatMap(() =>
            a.marquee.map(t => (
              <span key={Math.random()} style={{
                fontFamily: fonts.body, fontSize: '0.65rem',
      ...
      <div style={{ maxWidth: '1200px', margin: '0 auto', padding: '7rem 2rem' }}>
        ...
        >
          ...
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '2rem', alignItems: 'end' }}>
            <h2 style={{
              ...
              fontSize: 'clamp(3.5rem, 7vw, 8rem)', color: '#fff', lineHeight: 0.92, margin: 0,
            }}>
              {a.headline1}<br />
            ...
            <h2 style={{
              fontFamily: fonts.display, fontWeight: 300,
              fontSize: 'clamp(3.5rem, 7vw, 8rem)', color: 'rgba(255,255,255,0.18)',
        ...
        >
          {a.stats.map(({ num, label, sub }, i) => (
            ...
            >
              <div style={{ fontFamily: fonts.display, fontWeight: 300, fontSize: 'clamp(2.5rem, 4vw, 4.5rem)', color: '#fff', lineHeight: 1, marginBottom: '0.5rem' }}>
                {num}
              </div>
              <div style={{ fontFamily: fonts.body, fontSize: '0.75rem', color: 'rgba(255,255,255,0.7)', fontWeight: 500, marginBottom: '0.2rem' }}>
                {label}
              </div>
              <div style={{ fontFamily: fonts.body, fontSize: '0.6rem', letterSpacing: '0.15em', textTransform: 'uppercase', color: 'rgba(255,255,255,0.22)' }}>
                {sub}
              </div>
            </div>
          ))}
        ...
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '6rem', alignItems: 'start' }}>
          ...
          >
            <p style={{ fontFamily: fonts.display, fontWeight: 300, fontSize: 'clamp(1.2rem, 1.8vw, 1.6rem)', color: 'rgba(255,255,255,0.7)', lineHeight: 1.7, marginBottom: '1.5rem' }}>
              {a.body1}
              <em style={{ fontStyle: isRtl ? 'normal' : 'italic', color: '#fff' }}>{a.body1em}</em>
              {a.body1b}
            </p>
            <p style={{ fontFamily: fonts.body, fontSize: '0.85rem', color: 'rgba(255,255,255,0.38)', lineHeight: 1.9, marginBottom: '2.5rem' }}>
              {a.body2}
            </p>
            ...
            <a
              ...
              style={{
                ...
                borderBottom: '1px solid rgba(255,255,255,0.2)', paddingBottom: '0.3rem',
              }}
            >
              {a.cta}
          ...
          >
            {a.values.map(({ title, desc, icon }, i) => (
              <div
                ...
                style={{
                  padding: '1.8rem 0',
                  borderBottom: i < a.values.length - 1 ? '1px solid rgba(255,255,255,0.06)' : 'none',
                  display: 'grid', gridTemplateColumns: '2.5rem 1fr', gap: '1.2rem', alignItems: 'start',
                  opacity: valuesEl.vis ? 1 : 0,
                  transform: valuesEl.vis ? 'translateY(0)' : 'translateY(16px)',
                  transition: `opacity 0.6s ease ${0.4 + i * 0.1}s, transform 0.6s cubic-bezier(0.16,1,0.3,1) ${0.4 + i * 0.1}s`,
                }}
              >
                <span style={{ color: 'rgba(250,204,21,0.6)', fontSize: '1.1rem', marginTop: '0.15rem' }}>{icon}</span>
                <div>
                  <div style={{ fontFamily: fonts.body, fontSize: '0.82rem', fontWeight: 600, color: '#fff', marginBottom: '0.4rem', letterSpacing: '0.02em' }}>
                    {title}
                  </div>
                  <div style={{ fontFamily: fonts.body, fontSize: '0.78rem', color: 'rgba(255,255,255,0.35)', lineHeight: 1.65 }}>
                    {desc}
                  </div>
                </div>
              </div>
